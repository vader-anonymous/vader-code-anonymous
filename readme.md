...
[![Website](https://img.shields.io/badge/🌎-Anonymous_Website-blue.svg)](https://vader-anonymous.github.io/)
</div>
...
- We provided pretrained LoRA weights in folder `VADER/VADER-VideoCrafter/checkpoints/`. The `pretrained_lora_pickScore/vader_videocrafter_pickscore.pt` is the model fine-tuned using PickScore function on chatgpt_custom_animal.txt with LoRA rank of 16, while `pretrained_lora_hps_aesthetic/vader_videocrafter_hps_aesthetic.pt` is the model fine-tuned using a combination of HPSv2.1 and Aesthetic function on chatgpt_custom_instruments.txt with LoRA rank of 8.
